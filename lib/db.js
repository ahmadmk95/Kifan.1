...
function initSchema() {
  ...
  const unitCount = db.prepare('SELECT COUNT(*) AS n FROM fridge_units').get().n;
  if (unitCount === 0) {
    const ins = db.prepare('INSERT OR IGNORE INTO fridge_units (id, name) VALUES (?, ?)');
    for (const u of ['كيلو', 'غرام', 'قطعة', 'حبة', 'علبة', 'كيس', 'كرتون', 'صندوق', 'لتر', 'عبوة']) {
      ins.run(crypto.randomUUID(), u);
    }
  }
