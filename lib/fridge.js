...
export function listFridgeCategories() {
  return db.prepare('SELECT id, name FROM fridge_categories ORDER BY name COLLATE NOCASE').all();
}
  ...
  return db
    .prepare(
      `SELECT i.*, c.name AS category_name
       FROM fridge_items i
       LEFT JOIN fridge_categories c ON c.id = i.category_id
