...
export function getBalanceUsd(profileId, rates = getRates()) {
  ...
  for (const r of rows) {
    ...
    if (r.type === 'donation') bal += usd;
    else if (!r.pending) bal -= usd; // paid purchases only
  }
  return bal;
}
...
export function getAccounting(profileId) {
  ...
  let pendingUsd = 0;   // unpaid obligations
  let pendingCount = 0;
  const byCategory = {};
  // Native (un-converted) totals, one bucket per currency actually used.
  const byCurrency = {};
  const ccy = (c) => (byCurrency[c] ||= { currency: c, donations: 0, purchases: 0, pending: 0 });
  ...
  return {
    ...
    by_currency: CURRENCIES
      ...
      .map((c) => {
        ...
        return { currency: c, donations: b.donations, purchases: b.purchases, pending: b.pending, balance, projected: balance - b.pending };
      }),
