...
export const api = {
  ...
  // Accounting
  accounting: (profile) => request('/api/accounting' + (profile ? '?profile=' + encodeURIComponent(profile) : '')),
  ...
  payTransaction: (id) => request(`/api/accounting/transactions/${id}`, { method: 'PATCH', body: JSON.stringify({ pending: false }) }),
  addProfile: (name) => request('/api/accounting/profiles', { method: 'POST', body: JSON.stringify({ name }) }),
  renameProfile: (id, name) => request(`/api/accounting/profiles/${id}`, { method: 'PATCH', body: JSON.stringify({ name }) }),
  archiveProfile: (id) => request(`/api/accounting/profiles/${id}`, { method: 'DELETE' }),
  restoreProfile: (id) => request(`/api/accounting/profiles/${id}`, { method: 'PATCH', body: JSON.stringify({ archived: false }) }),
  reconciliations: (profile) => request('/api/accounting/reconcile' + (profile ? '?profile=' + encodeURIComponent(profile) : '')),
  saveReconciliation: (payload) => request('/api/accounting/reconcile', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  extractRecipe: (file) => {
    const fd = new FormData();
    fd.append('file', file);
    return fetch('/api/recipes/extract', { method: 'POST', body: fd }).then(async (r) => {
      const data = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(data.error || 'تعذّرت قراءة الصورة');
      return data;
    });
  },
