async function request(path, opts = {}) {
  const res = await fetch(path, {
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
    ...opts,
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    const err = new Error(data.error || 'حدث خطأ غير متوقع');
    err.status = res.status;
    err.data = data;
    throw err;
  }
  return data;
}

export const api = {
  me: () => request('/api/me'),
  login: (username, password) => request('/api/auth/login', { method: 'POST', body: JSON.stringify({ username, password }) }),
  register: (name, username, password) =>
    request('/api/auth/register', { method: 'POST', body: JSON.stringify({ name, username, password }) }),
  logout: () => request('/api/auth/logout', { method: 'POST' }),
  nights: () => request('/api/nights'),
  activeNight: () => request('/api/nights/active'),
  setActiveNight: (id) => request('/api/nights/active', { method: 'POST', body: JSON.stringify({ id }) }),
  myTasks: (nightId) => request(`/api/my-tasks?night=${nightId}`),
  allTasks: (nightId) => request(`/api/tasks?night=${nightId}`),
  addTask: (payload) => request('/api/tasks', { method: 'POST', body: JSON.stringify(payload) }),
  editTask: (id, payload) => request(`/api/tasks/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
  removeTask: (id) => request(`/api/tasks/${id}`, { method: 'DELETE' }),
  toggleTask: (id, done) => request(`/api/tasks/${id}/done`, { method: 'PATCH', body: JSON.stringify({ done }) }),
  claimTask: (id, claim) => request(`/api/tasks/${id}/claim`, { method: 'PATCH', body: JSON.stringify({ claim }) }),
  addComment: (id, text) => request(`/api/tasks/${id}/comments`, { method: 'POST', body: JSON.stringify({ text }) }),
  removeComment: (taskId, commentId) => request(`/api/tasks/${taskId}/comments/${commentId}`, { method: 'DELETE' }),
  committees: () => request('/api/committees'),
  addCommittee: (payload) => request('/api/committees', { method: 'POST', body: JSON.stringify(payload) }),
  removeCommittee: (id) => request(`/api/committees/${id}`, { method: 'DELETE' }),
  requests: () => request('/api/requests'),
  activateRequest: (id, committeeId) =>
    request(`/api/requests/${id}/activate`, { method: 'POST', body: JSON.stringify({ committee_id: committeeId }) }),
  rejectRequest: (id) => request(`/api/requests/${id}`, { method: 'DELETE' }),
  overview: (nightId) => request(`/api/overview?night=${nightId}`),
  comments: (nightId) => request(`/api/comments?night=${nightId}`),
  unseenCommentsCount: () => request('/api/comments/unseen-count'),
  chatMessages: (committeeId) => request(`/api/chat?committee=${committeeId}`),
  sendChatMessage: (committee_id, text) => request('/api/chat', { method: 'POST', body: JSON.stringify({ committee_id, text }) }),
  chatUnseenCount: () => request('/api/chat/unseen-count'),
  hasanat: () => request('/api/hasanat'),
  setCommitteeSupervisor: (committeeId, userId) =>
    request(`/api/committees/${committeeId}/supervisor`, { method: 'PATCH', body: JSON.stringify({ userId }) }),
  ratings: (memberId) => request(memberId ? `/api/ratings?member=${memberId}` : '/api/ratings'),
  allRatings: () => request('/api/ratings?all=1'),
  addRating: (memberId, rating, comment) =>
    request('/api/ratings', { method: 'POST', body: JSON.stringify({ memberId, rating, comment }) }),
  removeRating: (id) => request(`/api/ratings/${id}`, { method: 'DELETE' }),
  spotlight: () => request('/api/spotlight'),
  setSpotlight: (memberId, note) => request('/api/spotlight', { method: 'POST', body: JSON.stringify({ memberId, note }) }),
  modifySpotlight: (memberId, note) => request('/api/spotlight', { method: 'PATCH', body: JSON.stringify({ memberId, note }) }),
  cancelSpotlight: () => request('/api/spotlight', { method: 'DELETE' }),
  encourageSpotlight: () => request('/api/spotlight/encourage', { method: 'POST' }),
  users: () => request('/api/users'),
  addUser: (payload) => request('/api/users', { method: 'POST', body: JSON.stringify(payload) }),
  removeUser: (id) => request(`/api/users/${id}`, { method: 'DELETE' }),
  updateUser: (id, payload) => request(`/api/users/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
  resetPassword: (id, password) => request(`/api/users/${id}/password`, { method: 'PATCH', body: JSON.stringify({ password }) }),
};
