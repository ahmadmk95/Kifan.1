...
//   member + access=fridge      → لجنة التغذية: الثلاجة + دار الجيل (and can
//                                 also view the لجان)
...
export function canCommittees(user) {
  // Fridge users can also view the committee (اللجان) area.
  return !!user && user.status !== 'pending' &&
    (user.role === 'admin' || user.access === 'committees' || user.access === 'viewer' || user.access === 'fridge');
}
...
export function publicUser(u) {
  if (!u) return null;
  return { id: u.id, name: u.name, username: u.username, role: u.role, access: u.access, status: u.status, can_prepare: u.can_prepare ? 1 : 0 };
}
