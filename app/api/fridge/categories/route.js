...
export async function POST(req) {
  ...
  const exists = db.prepare('SELECT id FROM fridge_categories WHERE name = ? COLLATE NOCASE').get(name);
  if (exists) return NextResponse.json({ category: { id: exists.id, name } });

  const id = crypto.randomUUID();
  db.prepare('INSERT INTO fridge_categories (id, name) VALUES (?, ?)').run(id, name);
